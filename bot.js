...
client.on('ready', function(){
    ...
    setInterval(function (){
        ...
        i = i+j;
        client.user.setGame(setGame[%help],`http://www.twitch.tv/barontube`);
    }, ms);
...
if (message.author.id !== '518816831734022154') return message.reply('** هذا الأمر فقط لصاحب البوت و شكراًً **')
client.user.setGame(argresult);
  ...
  var invites = await message.guild.fetchInvites();
    ...
    invites.forEach(i => {
      ...
      if (i.uses === 30) {//يمديك تعدل رقم وصول العدد حق الانفايت الى اأقل أو أكثر
          message.member.addRole(message.member.guild.roles.find("name","⇁『Specialinviter』‏‏༄  ❥"))//هنآ أسم ألرتبه اللي تجيهه
.catch(RebeL =>{
console.log('`Error`: ' + RebeL);
});
}
if (i.uses === 15) {
message.member.addRole(message.member.guild.roles.find("name","⇁『Inviter』‏‏༄  ❥"))
.catch(RebeL =>{
console.log('`Error`: ' + RebeL);
});
}
if (i.uses === 5) {
message.member.addRole(message.member.guild.roles.find("name","⇁『lowerinviter』‏‏༄  ❥"))
.catch(RebeL =>{
console.log('`Error`: ' + RebeL);
});
...
client.on('message', message => {
if(message.content.includes("<@518816831734022154>")) {
...
client.login(NTY1ODA0MTAxODIxMjY3OTY4.XK7x6w.PNTcbltsEjHV5QANfM681dvaCfg)
